...
BotaoTexto::BotaoTexto(const std::string info, const sf::Vector2f tam, const sf::Vector2f pos, const IDs::ID ID, const sf::Color corSelecionado) : Botao(tam, pos, ID, TEMPO_TROCAR_COR), corSelecionado(corSelecionado), texto(pGG->carregarFonte(CAMINHO_FONTE), info), selecionado(false)
{
...
{
    atualizarAnimacao();
    pGG->desenhaElementos(texto.getTexto());
}
