...
Projetil::Projetil(const sf::Vector2f pos, const sf::Vector2f tam, const sf::Vector2f direcao, const float velocidade, const float dano, const float alcanceMaximo)
    : Entidade(pos, tam, VELOCIDADE_GRAVIDADE, IDs::ID::projetil), direcao(direcao), velocidade(0.3f), dano(8.f), distanciaPercorrida(0.f), alcanceMaximo(ALCANCE_MAX), ativo(true)
{
