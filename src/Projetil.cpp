...
Projetil::Projetil(const sf::Vector2f pos, const sf::Vector2f tam, const sf::Vector2f direcao, const float velocidade, const float dano, const float alcanceMaximo)
    : Entidade(pos, tam, VELOCIDADE_GRAVIDADE, IDs::ID::projetil), direcao(direcao), velocidade(velocidade), dano(dano), distanciaPercorrida(0.f), alcanceMaximo(ALCANCE_MAX), ativo(true)
{
